...
def reindex() -> None:
    ...
    # Read existing "context" metadata (Contextual Retrieval — see
    # rag/contextualizer.py) BEFORE clearing, so a from-scratch re-embed
    # (e.g. after switching embedding providers) doesn't silently drop
    # contexts already generated by scripts/contextualize_reindex.py.
    # Missing entirely (no contexts generated yet) -> {} -> every chunk
    # falls back to "" below, same as pre-Contextual-Retrieval behavior.
    existing_contexts = vs.get_contexts()
    if any(existing_contexts.values()):
        print(f"Preserving {sum(1 for v in existing_contexts.values() if v)} existing chunk context(s).")
    ...
    for batch_num, i in enumerate(range(0, len(chunks), BATCH_SIZE), start=1):
        ...
        contexts = [existing_contexts.get(cid, "") for cid in chunk_ids]
        ...
        vs.add_chunks(
            ...
            metadatas=[
                ...
                for c, ctx in zip(batch, contexts)
            ],
        )
